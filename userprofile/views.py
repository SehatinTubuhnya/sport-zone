...
def profile_page_view(request, username):
    ...
    context = {"profile_user_username": username}
    return render(request, "profile.html", context)


@require_http_methods(["GET"])
def profile_detail_api_view(request, username=None):
    if username is None:
    ...
    data = {
        ...
        "birth_date_display": _date(profile_user.birth_date, "d F Y")
        if profile_user.birth_date
        else "-",
...
def profile_content_api_view(request, user_id):
    ...
    if tab == "produk" and profile_user.is_seller:
        ...
        try:
            articles = (
                News.objects.filter(user=profile_user).all().order_by("-created_at")
            )
            serialized_data = [
                {
                    "id": a.id,
...
def profile_update_api_view(request):
    ...
    if new_password:
        if new_password != data.get("confirm_password"):
            return HttpResponseBadRequest(
                json.dumps({"error": "Password baru dan konfirmasi tidak cocok!"}),
    ...
    response_data = {
        ...
        "new_data": {
            ...
            "birth_date_display": _date(user.birth_date, "d F Y")
            if user.birth_date
            else "-",
        },
    }
